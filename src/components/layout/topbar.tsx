...
import { ORDER, ROLES } from "@/lib/constants"
...
export default function Topbar() {
  ...
  return (
    <header className="sticky top-0 z-20 flex h-[60px] items-center gap-3 border-b border-border bg-[hsl(40_33%_99%/0.85)] px-[22px] backdrop-blur-md">
      ...
      >
        <i className="ri-menu-line text-[17px]" />
      </button>
      ...
      <div className="relative ml-auto flex items-center gap-2">
        ...
        <div className="relative">
          ...
          >
            ...
            <span>{def.name}</span>
            {menuOpen ? (
              <i className="ri-arrow-up-s-line text-[15px] text-muted-fg" />
            ) : (
              <i className="ri-arrow-down-s-line text-[15px] text-muted-fg" />
            )}
          </button>
          {menuOpen && (
            <>
                ...
                {ORDER.map((k) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {k === role && <i className="ri-check-line text-[15px]" />}
                    </button>
                  )
                })}
