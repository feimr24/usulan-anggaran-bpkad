...
import { JENIS, SUB_KEGIATAN } from "@/lib/constants"
...
import { rupiah, today } from "@/lib/utils"
...
export default function VerifikasiUsulan() {
  ...
  const masuk = useMemo(
    () =>
      usulanList.filter(
        (u) => u.status === "diajukan" || u.status === "verifikasi"
      ),
    [usulanList]
  )
  ...
  return (
    <div>
            ...
            <table className="w-full border-collapse">
              ...
              <tbody>
                {masuk.length ? (
                  masuk.map((u) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-right">
                        <button
                          onClick={() => openVer(u)}
                          className="ml-auto inline-flex items-center gap-1.5 rounded-[7px] border-none bg-danger px-2.5 py-1.5 text-sm font-bold text-white shadow-sm2 hover:bg-[hsl(8_62%_40%)]"
                        >
                          <i className="ri-file-list-3-line h-[15px] w-[15px]" />
                          Verifikasi
                        </button>
                      </td>
                    </tr>
                  ...
                  <EmptyState
                    colSpan={5}
                    message="Tidak ada usulan menunggu verifikasi."
                  />
                )}
              </tbody>
            </table>
        ...
        <Card>
          <div className="overflow-x-auto">
            <table className="w-full border-collapse">
              <thead>
                <tr>
                  {["ID", "SKPD", "Tgl Verifikasi", "Hasil", "Nilai"].map(
                    (h, i) => (
                      <th
                        key={i}
                        className={`border-b border-border px-4 py-[11px] text-left text-xs font-semibold uppercase tracking-[0.04em] text-muted-fg ${
      ...
      <Dialog
        open={!!verifikasiFor}
        onOpenChange={() => setVerifikasiFor(null)}
      >
          ...
          {verifikasiFor && (
            <>
              <DialogHeader>
                ...
                <DialogDescription>
                  Periksa detail usulan, catat tanggal verifikasi, lalu proses
              ...
              <div className="space-y-4">
                ...
                <div className="rounded-lg border border-border bg-card p-4">
                  ...
                  {verifikasiFor.subKegiatanEntries &&
                  verifikasiFor.subKegiatanEntries.length > 0 ? (
                ...
                {/* Form verifikasi */}
                <div className="rounded-lg border border-border bg-card p-4">
                  <h3 className="mb-3 text-sm font-semibold">Form Verifikasi</h3>
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <select
                        value={jenis}
                        onChange={(e) =>
                          setJenis(e.target.value as JenisPersetujuan)
                        }
              ...
              <DialogFooter className="flex !flex-row !justify-between">
                ...
                >
                  <i className="ri-refresh-line h-[15px] w-[15px]" />
                  Revisi
                </button>
                <button
                  onClick={doProses}
                ...
                >
                  <i className="ri-checkbox-circle-line h-[15px] w-[15px]" />
                  Proses
                </button>
              </DialogFooter>
            </>
