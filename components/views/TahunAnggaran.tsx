...
import { TahunAnggaran } from "@/lib/types";
  ...
  function save() {
    ...
    if (editing) {
      setTahunAnggaranList((list) =>
        list.map((x) => (x.id === editing.id ? { ...x, tahun: tahun.trim() } : x))
      );
      toast("Tahun anggaran diperbarui");
    } else {
      setTahunAnggaranList((list) => [...list, { id: uid(), tahun: tahun.trim(), aktif: false }]);
  ...
  return (
    <div>
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="font-serif text-2xl font-medium tracking-tight">Tahun Anggaran</h1>
          <p className="mt-1 text-sm text-muted-fg">
            Kelola daftar tahun anggaran. Tahun yang tersedia akan muncul di form tambah jadwal usulan.
          </p>
        </div>
            ...
            <tbody>
              {tahunAnggaranList.length ? (
                tahunAnggaranList.map((t) => (
                  <tr key={t.id} className="border-b border-[hsl(34_16%_92%)] last:border-none hover:bg-[hsl(36_24%_97%)]">
      ...
      <Modal open={dialogOpen} onClose={() => setDialogOpen(false)}>
        <DialogHead
          title={editing ? "Edit Tahun Anggaran" : "Tambah Tahun Anggaran"}
          desc="Tahun anggaran akan digunakan di form jadwal usulan."
        />
        <DialogBody>
          <div>
            <label>Tahun</label>
